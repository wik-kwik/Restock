...
public interface ThresholdRepo extends ListCrudRepository<ThresholdEntity, Long> {
    ...
    @Query("SELECT te FROM ThresholdEntity te INNER JOIN SensorEntity se ON se.id = te.sensorEntity.id WHERE te.removeDate IS NULL AND te.type = 'U' AND se.sensorToken = :sensorToken")
